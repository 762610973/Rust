...
* [RUST学习🎉️](#rust)
  ...
  * [使用结构体组织关联数据](#)
  * [枚举和模式匹配](#)
  * [07|使用包、Crate和模块管理不断增长的项目](#07crate)
    * [包和crate](#crate)
    * [定义模块来控制作用域与私有性](#)
    * [路径用于引用模块树中的项](#)
    * [使用use关键字将名称引入作用域](#use)
    * [将模块分割进不同文件](#)
  * [常见集合](#)
    * [vector用来存储一系列的值](#vector)
    * [使用字符串存储UTF-8编码的文本](#utf-8)
    * [哈希map](#map)
<!-- TOC -->
...
- **Rust永远不会自动地创建数据的深度拷贝**.在Rust中,任何自动的赋值操作都可以被视为高效的.变量和数据交互的方式:克隆
...
- 集合指向的数据是存储在堆上的
  ...
  - 哈希map:key-value
### vector用来存储一系列的值
- vector使用泛型实现的,可以存放任意类型的值
- 丢弃vector时,也会丢弃其所有元素,起离开作用域时会被释放
- 通过索引或者get读取vector的元素
- get方法返回一个option
- 程序获取了一个有效的引用,借用检查器将会执行所有权和借用规则
### 使用字符串存储UTF-8编码的文本
- `String`类型是由标准库提供的,可增长、可变、有所有权、UTF-8编码的字符串类型
- `String`和字符串slice都是UTF-8编码
- `push_str`采用字符串slice,并不需要获取参数的所有权
- `&String`可以被强转成`&str`(解引用强制转换)
- Rust的字符串不支持索引
- Rust提供了多种不同的方式来解释计算机存储的原始字符串数据
  - 字节
  - 标量值
  - 字形簇
### 哈希map
- 哈希map的数据在堆上
- 一旦键值对被插入后就被哈希map所拥有,如果将值的引用插入哈希map,这些值本身不会被移动进哈希map中

## 错误处理
- ?运算符可被用于返回值类型为Result的函数
- 示例、代码原型和测试都非常适合panic
- 当代码有可能以有害状态结束时,建议让代码panic.